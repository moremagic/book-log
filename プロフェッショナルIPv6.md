...
- ゾーン；あるスコープを持つトポロジ的な範囲（RFC4007）
  - スコープはアドレス体系から決まる
  - ゾーンは個々のリンクを構成するインターフェースで決まる
- ゾーンの決まり方
  - インターフェースローカルスコープ
    - ノード内各インターフェースに一つのゾーン
    - マルチキャストのみ
  - リンクローカルスコープ
    - リンクと、接続されたインターフェースに一つのゾーン
    - ユニキャスト・エニーキャスト・マルチキャストの場合
  - グローバルスコープ
    - インターネット接続されたリンクとネットワークインターフェース１つのゾーン
  - 上記以外のスコープ
    - ネットワーク管理者によってゾーンが定義され設定される
- 同じスコープに対するゾーンがオーバラップすることはない
- IPv6 ではパケットはゾーン外に転送することは禁止されている
- ユニキャスト・エニーキャストに必要なゾーン
  - リンクローカルスコープに対応するもの
  - グローバルスコープに対応するもの
- マルチキャストに必要なゾーン
  - たくさんのゾーンが必要になる（11.4節で解説)

#### 4.7.1 ゾーンインデックス

- ゾーンとゾーンの境界；ノード内に存在する
- ノード内各インターフェースがゾーンとゾーン境界
- NICが同じIPv6アドレスを持ちながら別のゾーンで利用される場合もある（？？）
- ゾーンインデックス（ゾーンID）；あるインターフェースにおけるアドレスがどのゾーンに属しているかを識別するID
- ゾーンインデックスをどのように表現するかは RFC4007 で規定されていない
  - 実装によって異なる
  - BSD実装ではリンクとインターフェースが1対1だと仮定
  - 実際の都合上インターフェース番号をゾーンインデックスとして使用（簡易記法）
    - ec) fe80::1%3
    - ec) fe80::1%eth0

### 4.8 グローバルユニキャストアドレス

- グローバルユニキャストアドレスのフォーマット(RFC4291)
  - グローバルルーティングプレフィックス(nビット)
  - サブネットID(mビット)
  - インターフェース識別子(128-(n+m)ビット)
- IPv６ユニキャストアドレス
  - 最初の３ビットが`000` で開始するものを除く
  - インターフェース識別子を64ビット長にすることを要求している
  - ex:
    - グローバルルーティングプレフィックス；32ビット
    - サブネットID；32ビット
    - 仮定内個別機器識別のインターフェース識別用に残り64ビット

#### 4.8.1 IPv6アドレスの/64境界について

- IPv6 基本仕様ではIPv6アドレスの128ビットについてネットワーク境界を明記していない
  - サブネットプレフィックスとインターフェース識別子の境界について明記していない
- RFC4291 では 先頭3ビットが`000`で始まらないすべてのユニキャストアドレスのインターフェース識別子は64ビットであると規定
  - 例外として RFC6164 で規定された /127 を使う場合あり
- 64ビット境界について
  - RFC7421にまとめられている
    - 64ビット境界が広く採用されるようになった経緯
    - 関連するRFC
    - インターフェース識別子部分を可変長した場合に発生しうる課題などをまとめたRFC
  - /64 境界は IPv6 の初期デザイン時に議論されていた
  - /80 境界の案もあったが /64 で落ち着く
    - 48ビットのMACアドレスを置き換える形でEUI-64 のMACアドレスが普及するだろうと予想されていた
- IPv6 の基本使用はネットワークアドレスは64ビットに限定されていない
  - そのためネットワークアドレスに/64 以外を割り当てることは可能
  - しかし、、、
    - 一部プロトコルは /64 に対応できないものもある
    - 転送機器が /64 以外の /80 /96 に対応したものがある
      - 転送機器が /64 に対応できないものもある
      - /64 を超えると性能劣化するルータもある
- /64 を採用するのが無難
